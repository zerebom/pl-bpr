...
class MatrixData(ABC):
    def __init__(self, mat: sp.coo_matrix) -> None:
        pass


class Interactions(data.Dataset, MatrixData):
    """
...
class PairwiseInteractions(data.Dataset, MatrixData):
    """
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            if self.hogwild:
                ...
                for rank in range(self.num_workers):
                    p = mp.Process(
                        target=self._fit_epoch, kwargs={"epoch": epoch, "queue": queue}
                    )
                    p.start()
                    processes.append(p)
                for p in processes:
            ...
            if self.test is not None:
                ...
                for metric in self.eval_metrics:
                    ...
                    res = func(
                        self.model,
                        self.test_loader.dataset.mat_csr,
